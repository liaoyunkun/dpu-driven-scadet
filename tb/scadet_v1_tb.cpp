...
int main() {
    ...
    #else
    std::vector<BUS_TYPE> tagMem(TOTAL_SETS * TAG_BLK_NUM);
    #endif
    ...
    #else
    std::vector<BUS_TYPE> cntMem(TOTAL_SETS * CNT_BLK_NUM);
    #endif
    ...
    // generate inStream
    ap_shift_reg<meta_in_t, 100> shift_reg;
    meta_in_t shift_tsk_out;
    for (size_t i = 0; i < TRACE_CNT; i++) {
        // shift_tsk_out = shift_reg.shift(trace_set[i]);
        // for (size_t j = 0; j < 100; j++) {
        //     #pragma HLS pipeline II=1
        //     shift_tsk_out = shift_reg.shift(0);
        //     // std::cout << "j = " << j << ", shift_tsk_out = " << shift_tsk_out << std::endl; 
        // }
        
        // inStream.write(shift_tsk_out);
        inStream.write(trace_set[i]); 
        std::cout << "write a new tsk " << std::dec << i << std::endl;   
        #if (MULTI_TRAN_MODE == 1)
        #if (USE_TAPA == 0)
        scadet_v1(inStream, outStream, tagMem, cntMem);
        #else
        tapa::invoke(scadet_v1_top, FLAGS_bitstream, inStream, outStream, tapa::read_write_mmap<BUS_TYPE>(tagMem), tapa::read_write_mmap<BUS_TYPE>(cntMem));
        #endif
        #endif
    }
    #ifndef __SYNTHESIS__
    std::cout << "finish all task processing!" << std::endl;
    #endif
