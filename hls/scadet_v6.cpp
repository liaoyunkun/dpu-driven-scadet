...
)
{
    ...
    static bool isEvict;
    static bool find_attacker = false;

    static bool find_tag_in_list = false;
    static bool find_cnt_dqpn_in_list = false;
    static ap_uint<TAG_ELE_NUM> tag_valid_vec = 0;
    static ap_uint<TAG_ELE_NUM> tag_com_res_vec = 0;
    static ap_uint<CNT_ELE_NUM> cnt_valid_vec = 0;
    static ap_uint<CNT_ELE_NUM> cnt_com_res_vec = 0; 
    static BUS_TYPE search_cnt_line = 0;
    static bool evict_find_tag_in_list = false;
    static ap_uint<TAG_ELE_NUM> evict_tag_valid_vec = 0;
    static ap_uint<TAG_ELE_NUM> evict_tag_com_res_vec = 0;
    
    #if (OPEN_TREE_CNT_SER_OPT == 1) 
    static uint8    s1_evict_idx  [CNT_ELE_NUM/2] = {0};
    static uint8    s1_evict_idy  [CNT_ELE_NUM/2] = {0};
    #pragma HLS ARRAY_PARTITION variable=s1_evict_idx complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s1_evict_idy complete dim=0
    static uint8    s2_evict_idx  [CNT_ELE_NUM/4] = {0};
    static uint8    s2_evict_idy  [CNT_ELE_NUM/4] = {0};
    #pragma HLS ARRAY_PARTITION variable=s2_evict_idx complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s2_evict_idy complete dim=0
    static uint8    s3_evict_idx  [CNT_ELE_NUM/8] = {0};
    static uint8    s3_evict_idy  [CNT_ELE_NUM/8] = {0};
    #pragma HLS ARRAY_PARTITION variable=s3_evict_idx complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s3_evict_idy complete dim=0
    static uint8    s4_evict_idx  = 0; 
    static uint8    s4_evict_idy  = 0;

    static uint8 s0_cnt_ele_temp      [CNT_ELE_NUM] = {0};
    static QPN_TYPE  s0_cnt_ele_dqpn_temp [CNT_ELE_NUM] = {0};
    #pragma HLS ARRAY_PARTITION variable=s0_cnt_ele_temp complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s0_cnt_ele_dqpn_temp complete dim=0

    static uint8 s1_cnt_ele_temp      [CNT_ELE_NUM/2] = {0};
    static QPN_TYPE  s1_cnt_ele_dqpn_temp [CNT_ELE_NUM/2] = {0};
    #pragma HLS ARRAY_PARTITION variable=s1_cnt_ele_temp complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s1_cnt_ele_dqpn_temp complete dim=0

    static uint8 s2_cnt_ele_temp      [CNT_ELE_NUM/4] = {0};
    static QPN_TYPE  s2_cnt_ele_dqpn_temp [CNT_ELE_NUM/4] = {0};
    #pragma HLS ARRAY_PARTITION variable=s2_cnt_ele_temp complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s2_cnt_ele_dqpn_temp complete dim=0

    static uint8 s3_cnt_ele_temp      [CNT_ELE_NUM/8] = {0};
    static QPN_TYPE  s3_cnt_ele_dqpn_temp [CNT_ELE_NUM/8] = {0};
    #pragma HLS ARRAY_PARTITION variable=s3_cnt_ele_temp complete dim=0
    #pragma HLS ARRAY_PARTITION variable=s3_cnt_ele_dqpn_temp complete dim=0

    static uint8 s4_cnt_ele_temp      = 0;
    static QPN_TYPE  s4_cnt_ele_dqpn_temp = 0;
    #endif
        ...
        if (hit_set == 1) {
            ...
            if (ts_diff < INTRA_GROUP_TH) {
                ...
                for (unsigned int i = 0; i < tag_loop_iter_cnt; i++) {
                    ...
                    for (unsigned int j = 0; j < TAG_ELE_NUM; j++) {
                        #pragma HLS unroll
                        tag_valid_vec[j] = ((i*TAG_ELE_NUM+j) < uni_tags);
                    }
                    for (unsigned int j = 0; j < TAG_ELE_NUM; j++) {
                        #pragma HLS unroll
                        tag_ele_tag_temp = tags_line.range(((j+1) * TAG_ELE_LEN) -1, j * TAG_ELE_LEN + QPN_LEN);
                        tag_ele_dqpn_temp = tags_line.range(j * TAG_ELE_LEN + QPN_LEN - 1, j * TAG_ELE_LEN);
                        tag_com_res_vec[j] = tag_valid_vec[j] && (tag_ele_tag_temp == new_tag) && (tag_ele_dqpn_temp == new_dqpn);
                        #ifndef __SYNTHESIS__
                        // std::cout << "tag_ele_tag_temp = " << tag_ele_tag_temp << std::endl;
                        // std::cout << "tag_ele_dqpn_temp = " << tag_ele_dqpn_temp << std::endl;
                        // std::cout << "new_tag = " << new_tag << std::endl;
                        // std::cout << "new_dqpn = " << new_dqpn << std::endl;
                        #endif
                    }
                    if (tag_com_res_vec.or_reduce()) {
                        tag_hit = true;
                        break;
                    }
                }
                ...
                for (unsigned int i = 0; i < cnt_loop_iter_cnt; i++) {
                    #pragma HLS loop_flatten off
                    #pragma HLS pipeline II=1
                    search_cnt_line = cnt_dataArray[cache_evict_idx][i];
                    for (unsigned int j = 0; j < CNT_ELE_NUM; j++) {
                        #pragma HLS unroll
                        cnt_valid_vec[j] = ((i*CNT_ELE_NUM+j) < uni_dqpns);
                    }
                    for (unsigned int j = 0; j < CNT_ELE_NUM; j++) {
                        #pragma HLS unroll
                        tag_ele_dqpn_temp = search_cnt_line.range((j+1)*CNT_ELE_LEN-1, j * CNT_ELE_LEN + 8);
                        cnt_com_res_vec[j] = cnt_valid_vec[j] && (tag_ele_dqpn_temp == new_dqpn);
                    }
                    if (cnt_com_res_vec.or_reduce()) {
                        dqpn_hit = true;
                        for (unsigned j = 0; j < CNT_ELE_NUM; j++) {
                            #pragma HLS unroll
                            if (cnt_com_res_vec[j]) {
                                cnt_insert_idx = i;
                                cnt_insert_idy = j;
                                cnt_insert_val = search_cnt_line.range(j * CNT_ELE_LEN + 7, j * CNT_ELE_LEN) + 1;
                                dqpn_hit_idx = i;
                                dqpn_hit_idy = j;
                                dqpn_hit_cnt = search_cnt_line.range(j * CNT_ELE_LEN + 7, j * CNT_ELE_LEN);
                                break; 
                            }
                        }
                        break;
                ...
                if (!tag_hit && uni_tags == WAYS_NUM) {
                    ...
                    for (unsigned int i = 0; i < cnt_loop_iter_cnt; i++) {
                        ...
                        for (unsigned int j = 0; j < CNT_ELE_NUM; j++) {
                            #pragma HLS unroll
                            // s0_cnt_ele_temp[j] = ((i*CNT_ELE_NUM+j) < uni_dqpns)? cnt_line.range(j*CNT_ELE_LEN + 8-1, j*CNT_ELE_LEN) : 255;
                            s0_cnt_ele_temp[j] = ((i*CNT_ELE_NUM+j) >= uni_dqpns || (dqpn_hit && dqpn_hit_idx == i && dqpn_hit_idy == j))? 255 : cnt_line.range(j*CNT_ELE_LEN + 8-1, j*CNT_ELE_LEN);
                            s0_cnt_ele_dqpn_temp[j] = cnt_line.range(((j+1) * CNT_ELE_LEN)-1, j*CNT_ELE_LEN + 8);
                        }
                        // First Stage: 16 -> 8
                        for (unsigned int j = 0; j < CNT_ELE_NUM/2; j++) {
                            #pragma HLS unroll
                            if (s0_cnt_ele_temp[j*2] <= s0_cnt_ele_temp[j*2+1]) {
                                s1_cnt_ele_temp[j]      = s0_cnt_ele_temp[j*2];
                                s1_cnt_ele_dqpn_temp[j] = s0_cnt_ele_dqpn_temp[j*2];
                                s1_evict_idx[j]         = i;
                                s1_evict_idy[j]         = j*2;
                            } else {
                                s1_cnt_ele_temp[j]      = s0_cnt_ele_temp[j*2+1];
                                s1_cnt_ele_dqpn_temp[j] = s0_cnt_ele_dqpn_temp[j*2+1];
                                s1_evict_idx[j]         = i;
                                s1_evict_idy[j]         = j*2+1;
                            }
                        }
                        // Second Stage: 8 -> 4
                        for (unsigned int j = 0; j < CNT_ELE_NUM/4; j++) {
                            #pragma HLS unroll
                            if (s1_cnt_ele_temp[j*2] <= s1_cnt_ele_temp[j*2+1]) {
                                s2_cnt_ele_temp[j]      = s1_cnt_ele_temp[j*2];
                                s2_cnt_ele_dqpn_temp[j] = s1_cnt_ele_dqpn_temp[j*2];
                                s2_evict_idx[j]         = s1_evict_idx[j*2];
                                s2_evict_idy[j]         = s1_evict_idy[j*2];
                            } else {
                                s2_cnt_ele_temp[j]      = s1_cnt_ele_temp[j*2+1];
                                s2_cnt_ele_dqpn_temp[j] = s1_cnt_ele_dqpn_temp[j*2+1];
                                s2_evict_idx[j]         = s1_evict_idx[j*2+1];
                                s2_evict_idy[j]         = s1_evict_idy[j*2+1];
                            }
                        }
                        // Third Stage: 4 -> 2
                        for (unsigned int j = 0; j < CNT_ELE_NUM/8; j++) {
                            #pragma HLS unroll
                            if (s2_cnt_ele_temp[j*2] <= s2_cnt_ele_temp[j*2+1]) {
                                s3_cnt_ele_temp[j]      = s2_cnt_ele_temp[j*2];
                                s3_cnt_ele_dqpn_temp[j] = s2_cnt_ele_dqpn_temp[j*2];
                                s3_evict_idx[j]         = s2_evict_idx[j*2];
                                s3_evict_idy[j]         = s2_evict_idy[j*2];
                            } else {
                                s3_cnt_ele_temp[j]      = s2_cnt_ele_temp[j*2+1];
                                s3_cnt_ele_dqpn_temp[j] = s2_cnt_ele_dqpn_temp[j*2+1];
                                s3_evict_idx[j]         = s2_evict_idx[j*2+1];
                                s3_evict_idy[j]         = s2_evict_idy[j*2+1];
                            }
                        }
                        // Fourth Stage: 2 -> 1
                        for (unsigned int j = 0; j < CNT_ELE_NUM/16; j++) {
                            #pragma HLS unroll
                            if (s3_cnt_ele_temp[j*2] <= s3_cnt_ele_temp[j*2+1]) {
                                s4_cnt_ele_temp      = s3_cnt_ele_temp[j*2];
                                s4_cnt_ele_dqpn_temp = s3_cnt_ele_dqpn_temp[j*2];
                                s4_evict_idx         = s3_evict_idx[j*2];
                                s4_evict_idy         = s3_evict_idy[j*2];
                            } else {
                                s4_cnt_ele_temp      = s3_cnt_ele_temp[j*2+1];
                                s4_cnt_ele_dqpn_temp = s3_cnt_ele_dqpn_temp[j*2+1];
                                s4_evict_idx         = s3_evict_idx[j*2+1];
                                s4_evict_idy         = s3_evict_idy[j*2+1];
                            }
                        }
                        // Final Stage: Compare with the result of previous stage
                        if (s4_cnt_ele_temp <= evict_cnt) {
                            evict_cnt = s4_cnt_ele_temp;
                            evict_dqpn = s4_cnt_ele_dqpn_temp;
                            evict_idx = s4_evict_idx;
                            evict_idy = s4_evict_idy;
                        }
                        #endif
                    }
                    #ifndef __SYNTHESIS__
                    ...
                    for (unsigned int i = 0; i < tag_loop_iter_cnt && !find_evict_tag; i++) {
                        ...
                        for (unsigned int j = 0; j < TAG_ELE_NUM; j++) {
                            #pragma HLS unroll
                            evict_tag_valid_vec[j] = ((i*TAG_ELE_NUM+j) < uni_tags);
                        }
                        for (unsigned int j = 0; j < TAG_ELE_NUM; j++) {
                            #pragma HLS unroll
                            tag_ele_dqpn_temp = tags_line.range(j * TAG_ELE_LEN + QPN_LEN - 1, j * TAG_ELE_LEN);
                            evict_tag_com_res_vec[j] = evict_tag_valid_vec[j] && (tag_ele_dqpn_temp == new_dqpn);
                        }
                        if (evict_tag_com_res_vec.or_reduce()) {
                            find_evict_tag = true;
                            for (unsigned int j = 0; j < TAG_ELE_NUM; j++) {
                                #pragma HLS unroll
                                if (evict_tag_com_res_vec[j] == 1) {
                                    tags_evict_idx = i;
                                    tags_evict_idy = j;
                                    break;
                                }
